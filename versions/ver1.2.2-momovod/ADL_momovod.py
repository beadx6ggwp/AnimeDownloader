...
def downloadAnime(url, start=0, end=9999, threadNum=64):
    ...
    for eptitle, epUrl in zip(animeContent['eptitle'], animeContent['videoRequest']):
        ...
        download_m3u8(
            videoContent['videoUrl'], videoContent['header'], partDir, threadNum=threadNum)
...
def download_m3u8(m3u8data, header, directory='', threadNum=20):
    ...
    for i in range(0, threadNum):
        task = threading.Thread(target=thread_download,
                                args=(taskQueue, jobstatus))
        task.start()
        threads.append(task)
...
def thread_download(taskQueue, jobstatus):
    while not taskQueue.empty():
        ...
        state = download(job['href'],
                         headers=job['header'],
                         file_name=job['filename'],
                         directory=job['directory'],
                         jobstatus=jobstatus)
        if(not state and job not in taskQueue.queue):
            # print("retry:"+job['filename'])
            taskQueue.put(job)
    pass
...
def showStatus(jobstatus):
    while True:
        ...
        text = ' {0} / {1} | {2:>5.2f} MB |{3:>5.2f} MB/s'.format(
            nowep, allep, nowdl / (2**20), speed)
...
if __name__ == '__main__':
    ...
    # downloadAnime('https://www.momovod.com/vod-detail-id-74774.html', start=1, end=3)
    main(sys.argv)
